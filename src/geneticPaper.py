...
def solve(problem_grid, population_size=1000, selection_rate=0.5, max_generations_count=1000, mutation_rate=0.05):
    ...
    def stringify(population_args):
        result = ""
        for i in range(len(population_args)):
            for j in range(len(population_args[i])):
                result += str(population_args[i][j])
        return result
    ...
    for i in range(max_generations_count):
        ...
        string_representation = stringify(population[0])
        print(fitness(population[0]), string_representation)
